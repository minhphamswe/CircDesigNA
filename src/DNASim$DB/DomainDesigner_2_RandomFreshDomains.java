...
public class DomainDesigner_2_RandomFreshDomains {
	public static class DDSeqDesigner_2_ByRandomMutation_Client implements DDSeqDesigner{
		...
		DomainDesigner_2_RandomFreshDomains r;
		{
			r = new DomainDesigner_2_RandomFreshDomains();
		}
		private List<String> inputStrands;
		public DDSeqDesigner_2_ByRandomMutation_Client(List<String> inputStrands){
		...
		private SeqDesignerOption rule_ming_option = new SeqDesignerOption(){
			public String getDescription() {
				return "Use less C/G than A/T";
			}
			public boolean getState() {
				return r.rule_ming==1;
			}
			public void toggle() {
				r.rule_ming = 1-r.rule_ming;
			}
		};
	...
	public static DDSeqDesigner makeDesigner(List<String> inputStrands){
		return new DDSeqDesigner_2_ByRandomMutation_Client (inputStrands);
	}
	...
	public static void main(String[] args) throws Throwable{
		...
		DDSeqDesigner dsd = makeDesigner(strands);
		dsd.resume();
		Thread.sleep(50000);
		dsd.pause();
		Thread.sleep(10);
	...
	int rule_4g, rule_6at, rule_ccend, rule_ming, rule_init, rule_lockold, rule_targetworst, rule_gatc_avail;
	{
		...
		rule_ming = 0; // design tries to minimize usage of G
	...
	void main(int num_domain, int[] domain_length, int TOTAL_ATTEMPTS, Map<Integer, String> lockedDomains, List<DomainSequence> seqToSynthesize) {
		...
		while (true && !abort) {
			...
			mutateUntilValid(mut_domain,domain,domain_length,domain_gatc_avail,seqToSynthesize);
			...
			crosstalk[mut_domain][mut_domain] = 0;
			float junctionPenalties = 0,interactionSum=0,crosstalkSum=0, selfCrossTalk = 0;
				...
				for(DomainSequence seq : seqToSynthesize){
						...
						for(DomainSequence seq2 : seqToSynthesize){
							...
							if (!checkComplementary(seq,seq2)){
								double val = pairscore(seq,seq2,domain) * (JunctionPenalty_FullPenalty? 1 : 1f/(seqToSynthesize.size()*seqToSynthesize.size()));
				
								debugPenalty(val,"Junction-Junction Interaction",domain,seq,seq2);
								
								//displaySequence(seq, domain);
								//displaySequence(seq2, domain);
								//System.out.println(val);
								
								junctionPenalties += val;
								crosstalk[mut_domain][mut_domain] += val ;
							}
						}
			...
			for(i = 0; i < domain.length; i++){
				selfCrossTalk += domain_intrinsic[i];
			}
				
			// Domain score is <strikeout>max</strikeout SUM of interaction and crosstalk scores
			for(i = 0; i < num_domain; i++){
			...
			for(j = 0; j < num_domain; j++){
				for(i = 0; i < num_domain; i++){
					interactionSum += interaction[i][j];
					crosstalkSum += crosstalk[i][j];
				}
			}
			...
			*/
			...
			if (revert_mutation){
				...
				for (k = domain_length[mut_domain]-1; k >= 0; k--)
					domain[mut_domain][k] = old_domain[k];
	 ...
	 */
	private void mutateUntilValid(int mut_domain, int[][] domain,
			int[] domain_length, int[] domain_gatc_avail, List<DomainSequence> seqToSynthesize) {

		// Save old domain
		int len = domain_length[mut_domain];
		if (!(old_domain!=null && len <= old_domain.length)){
			old_domain = new int[len];
		}
		System.arraycopy(domain[mut_domain],0,old_domain,0,len);
		
		int[] mut_old = old_domain;
		...
		mutating: do {
			...
			for (int k = 0; k < num_mut; k++) {
				...
				if (domain[mut_domain][j] > 10) {
					...
					if ((domain_gatc_avail[mut_domain] & 1)+(domain_gatc_avail[mut_domain] & 2)/2+(domain_gatc_avail[mut_domain] & 4)/4+(domain_gatc_avail[mut_domain] & 8)/8 > 1) {
						int percentG, percentA, percentT, percentC;
						if (rule_ming==1) {
						...
						mut_new[j] = int_urn(1,100);
						if (mut_new[j] < percentG)
							mut_new[j] = G;
						else if (mut_new[j] < percentA)
							mut_new[j] = A;
						else if (mut_new[j] < percentT)
							mut_new[j] = T;
						else
							mut_new[j] = C;
						// Undo mutation if new base is not allowed
						if ((mut_new[j] == 1)&&(domain_gatc_avail[mut_domain] / 8 == 0))
							mut_new[j] = mut_old[j];
						if ((mut_new[j] == 2)&&((domain_gatc_avail[mut_domain] / 4)%2 == 0))
							mut_new[j] = mut_old[j];
						if ((mut_new[j] == 3)&&((domain_gatc_avail[mut_domain] / 2)%2 == 0))
							mut_new[j] = mut_old[j];
						if ((mut_new[j] == 4)&&(domain_gatc_avail[mut_domain] % 2== 0))
							mut_new[j] = mut_old[j];
			...
			if (!isValidSequenceSetup(mut_domain,seqToSynthesize,domain)){
				...
				//Revert
				System.arraycopy(old_domain,0,domain[mut_domain],0,len);
				continue mutating;
			}
			break;
		} while(true);
	...
	private boolean isValidSequenceSetup(int mut_domain, List<DomainSequence> seqToSynthesize, int[][] domain) {
		...
		if (!isValidSequence(mutateSequence,domain)){
			return false;
		}
		//Check all junctions
		for(DomainSequence seq : seqToSynthesize){
	 ...
	 * the input sequence.
	 */
	private boolean isValidSequence(DomainSequence seq, int[][] domain) {
		int k,q,base;
	...
	private void updateDomainScore(double[] domain_score, int i, int num_domain,
		...
		for (int j = 0; j < num_domain; j++) {
			domain_score[i] = Math.max(domain_score[i],interaction[i][j] + crosstalk[i][j] + domain_importance[i] + domain_importance[j]);
	...
	private void addIntrinsicScore(DomainSequence seq, int mut_domain, int[][] domain, double[] domain_intrinsic, int rule_4g, int rule_6at) {
		double val = selfcrosstalk(seq,domain);
		
		domain_intrinsic[mut_domain] += val;
		
		debugPenalty(val,"Self folding",domain,seq);
	}
	...
	private static final int debugOutTopScores = 5;
