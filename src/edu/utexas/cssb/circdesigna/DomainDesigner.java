...
import edu.utexas.cssb.circdesigna.DesignIntermediateReporter.DesignIntermediateScore;
import edu.utexas.cssb.circdesigna.SequenceDesigner.AlternativeResult;

import DnaDesign.AbstractDesigner.BlockDesigner;
import DnaDesign.AbstractDesigner.TinyGADesigner;
...
public abstract class DomainDesigner extends CircDesigNASystemElement{
	 ...
	 * 
	 * Why does this class exist? The DomainDesigner did not expose an API which made GUI
	 * writing as easy as it could be. To make client code simpler, this class has some adaptor methods (especially a constructor) 
	 * that will transform your input before feeding it to a DomainDesigner. 
	 * 
	 * Error checking is deferred to the DomainDesigner class itself.
	 * 
	...
	public static class DomainDesigner_Public implements SequenceDesigner<CircDesigNAOptions>{
		...
		public DomainDesigner_Public(List<String> inputStrands, String domainDefsBlock, CircDesigNAConfig Std){
			...
			r = new DomainDesignerImpl(new CircDesigNAMCSFolder(Std), Std);
			runOnStart.run();
		...
		private void printSequence(StringBuffer sb, DomainDesigner r2, DomainSequence ds, boolean withSeperator, String[] domains) {
			if (withSeperator)
				sb.append("[");
	...
	private class DomainDesignerAlternativeResult extends AlternativeResult{
		...
		public DomainDesignerAlternativeResult(String description, DomainDesignPMemberImpl best, AbstractDomainDesignTarget designTarget, DomainDefinitions dsd) {
			breakdown = DomainDesigner_SharedUtils.getScoreBreakdown(best.domain,designTarget,best.penalties);
	...
	private void fullScoreReport(PopulationDesignMember<DomainDesignPMemberImpl>[] population, DesignIntermediateReporter DIR, AbstractDomainDesignTarget designTarget, DomainDefinitions dsd){
		DomainDesignPMemberImpl best = DomainDesigner_SharedUtils.getBestMember(population);
		...
		DIR.endScoreReport();
		DesignScoreBreakdown outputScore = DomainDesigner_SharedUtils.getScoreBreakdown(best.domain,designTarget,best.penalties);
		...
		DomainDesignPMemberImpl farthest = DomainDesigner_SharedUtils.getFarthestFrom(best,population);
		...
		for(int i = 0; i < population.length; i++){
			tmp_alternatives[2+i] = new DomainDesignerAlternativeResult("Population ID "+i, (DomainDesignPMemberImpl)population[i], designTarget, dsd);
		}
	...
	int main(int num_domain, int[] domain_length, int TOTAL_ATTEMPTS, Map<Integer, String> lockedDomains, Map<Integer, String> initial, Map<Integer, String> positionConstraints, Map<Integer, DesignerCode> mutators, AbstractDomainDesignTarget designTarget, DesignIntermediateReporter DIR, DomainDefinitions dsd) {
		...
		try {
			...
			if (options.standardUseGA.getState()){
				dbesign = new TinyGADesigner(dbesignSingle, new DomainDesignerImpl.DParetoSort(), true);
			} else {
