...
import DnaDesignGUI.math.CircumscribedPolygonTool;
...
public class DomainStructureBNFTree implements AbstractComplex{
	...
	private static class ParserState {
		...
		void parse(){
			Result t = CDNA2PublicParser.parse(structure);
		...
		public void output() {
			...
			out.outerCurve = new CircumscribedPolygon();
			int i = 0;
			...
			for(DomainStructure struct : out2.values()){
				...
				DomainStructure.getOuterLevelSpace(S,struct,out.domainDefs.domainLengths,out.domains);
				
				if (struct instanceof HairpinStem){
					numHairpinsOnOuterLoop++;
				}
			}
			out.outerCurve.S = new float[S.size()];
			for(int k = 0; k < S.size(); k++){
				out.outerCurve.S[k] = S.get(k);
			}
			CircumscribedPolygonTool.solvePolygonProblem(out.outerCurve);
	...
	*/
	...
	public static class DomainStructure {
		...
		public static boolean getOuterLevelSpace(List<Float> holder, DomainStructure q,int[] domainLengths, int[] domains) {
			if (q instanceof HairpinStem){
				if (holder!=null) holder.add(HairpinStem.openingSize);
				return true;
				...
				if (holder!=null) holder.add(ThreePFivePOpenJunc.size);
				return true;
			}
	...
	public static class HairpinStem extends DomainStructure {
		public static float openingSize = 2;
		...
		public void handleSubConformation(int[] domainLengths, int[] domains) {
			...
			if (subStructure.size()>0){
				int index = 0;
				loop:for(DomainStructure q : subStructure){
					if (q instanceof SingleStranded && getOuterLevelSpace(null, q, domainLengths, domains)){
						isEmptyLoop = false;
					}
